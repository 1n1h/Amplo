...
export async function sendLeadNotification(lead: LeadEmailInput): Promise<void> {
  ...
  const subject = company
    ? `New lead: ${lead.name} from ${company}`
    : `New lead: ${lead.name}`;
...
export async function sendBookingEmails(input: BookingEmailInput): Promise<void> {
  ...
  const internalHtml = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px; color: #0b1936; background: #ede5d2;">
      ...
      <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
        ...
        ${input.industry ? `<tr><td style="padding:6px 0; color:#6a7390;">Industry</td><td style="padding:6px 0;">${escapeHtml(input.industry)}</td></tr>` : ''}
      </table>
      ${input.notes ? `<div style="margin-top: 20px; padding: 16px; background: #ffffff; border-left: 3px solid #c9a961; white-space: pre-wrap; font-size: 14px; line-height: 1.5;">${escapeHtml(input.notes)}</div>` : ''}
  ...
  const internalRes = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      subject: `New booking: ${input.name} at ${when}`,
      text: internalText,
      html: internalHtml,
    }),
  });
